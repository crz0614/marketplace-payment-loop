...
The reconciliation queue can be filtered server-side by an owned shop and by all, open, investigating or ignored exceptions. Both filters also apply to CSV export; invalid shop IDs and unknown action states are rejected before querying PostgreSQL. Migration `20260904163000_commerce_inventory_exception_shop_filter.sql` tracks the restricted three-argument RPC.
