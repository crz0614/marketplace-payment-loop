...
const orderCsv = (rows: any[]) => '\uFEFF'+[
  ['shop','channel','external_order_id','sku','quantity','amount','currency','status','occurred_at'],
  ...rows.map(x=>[x.shop?.name,x.shop?.channel,x.external_order_id,x.sku,x.quantity,(x.amount_minor/100).toFixed(2),x.currency,x.status,x.occurred_at])
...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/orders') { const shopId=url.searchParams.get('shop_id')||'',status=(url.searchParams.get('status')||'').trim(),from=url.searchParams.get('from')||'',to=url.searchParams.get('to')||'',format=url.searchParams.get('format')||'';if(shopId&&!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(shopId))return fail('invalid_shop');if(status.length>80)return fail('invalid_status');if((from&&!validDay(from))||(to&&!validDay(to))||(from&&to&&from>to))return fail('invalid_date_range');if(format&&format!=='csv')return fail('invalid_format');let query=db.from('vco_order_lines').select('id,shop_id,external_order_id,sku,quantity,amount_minor,currency,status,occurred_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('occurred_at',{ascending:false}).limit(200);if(shopId)query=query.eq('shop_id',shopId);if(status)query=query.eq('status',status);if(from)query=query.gte('occurred_at',from+'T00:00:00.000Z');if(to)query=query.lt('occurred_at',new Date(Date.parse(to+'T00:00:00.000Z')+86400000).toISOString());const {data,error}=await query;if(error)throw error;if(format==='csv')return new Response(orderCsv(data||[]),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="vesper-commerce-orders.csv"','cache-control':'no-store'}});return json({orders:data}) }
