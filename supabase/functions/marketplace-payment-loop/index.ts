...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/inventory') { const shopId=url.searchParams.get('shop_id')||'',low=url.searchParams.get('low_stock')||'';if(shopId&&!uuidPattern.test(shopId))return fail('invalid_shop');if(low&&low!=='true')return fail('invalid_low_stock');let query=db.from('vco_inventory').select('id,shop_id,sku,available_quantity,reorder_point,is_low_stock,source_name,captured_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('updated_at',{ascending:false}).limit(200);if(shopId)query=query.eq('shop_id',shopId);if(low)query=query.eq('is_low_stock',true);const {data,error}=await query;if(error)throw error;return json({inventory:data}) }
