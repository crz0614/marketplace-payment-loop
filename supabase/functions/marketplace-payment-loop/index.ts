...
const validDay = (value: string) => { const time=Date.parse(value+'T00:00:00.000Z');return /^\d{4}-\d{2}-\d{2}$/.test(value) && Number.isFinite(time) && new Date(time).toISOString().slice(0,10) === value }
const csvCell = (value: unknown) => { const text=String(value ?? ''),safe=/^[=+\-@]/.test(text)?"'"+text:text;return `"${safe.replaceAll('"','""')}"` }
const orderCsv = (rows: any[]) => '\uFEFF'+[
  ['shop','channel','external_order_id','sku','quantity','amount','currency','canonical_status','source_status','occurred_at'],
  ...rows.map(x=>[x.shop?.name,x.shop?.channel,x.external_order_id,x.sku,x.quantity,(x.amount_minor/100).toFixed(2),x.currency,x.canonical_status,x.status,x.occurred_at])
].map(row=>row.map(csvCell).join(',')).join('\r\n')
const inventoryCsv = (rows: any[]) => '\uFEFF'+[
  ['shop','channel','sku','available_quantity','reorder_point','source_name','captured_at'],
  ...rows.map(x=>[x.shop?.name,x.shop?.channel,x.sku,x.available_quantity,x.reorder_point,x.source_name,x.captured_at])
].map(row=>row.map(csvCell).join(',')).join('\r\n')
const inventoryExceptionCsv = (rows: any[]) => '\uFEFF'+[
  ['shop','channel','sku','order_line_count','ordered_quantity','action_status','action_note','last_order_at','action_updated_at'],
  ...rows.map(x=>[x.shop_name,x.channel,x.sku,x.order_line_count,x.ordered_quantity,x.action_status||'open',x.action_note,x.last_order_at,x.action_updated_at])
].map(row=>row.map(csvCell).join(',')).join('\r\n')
...
const commerceInventory = (value: unknown) => {
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new HttpError('invalid_inventory')
  const x=value as Record<string,unknown>, sku=typeof x.sku==='string'?x.sku.trim():'', available=Number(x.available_quantity), reorder=Number(x.reorder_point)
  if(!sku||sku.length>120||!Number.isSafeInteger(available)||available<0||available>2147483647||!Number.isSafeInteger(reorder)||reorder<0||reorder>2147483647)throw new HttpError('invalid_inventory')
  return {sku,available_quantity:available,reorder_point:reorder}
}
...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/imports') { const {data,error}=await db.from('vco_imports').select('id,shop_id,source_name,row_count,created_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('created_at',{ascending:false}).limit(100);if(error)throw error;return json({imports:data}) }
    if (req.method === 'GET' && path === '/api/commerce/summary') { const {data,error}=await db.rpc('vco_order_summary',{p_owner:user.id});if(error)throw error;return json({summary:data}) }
    if (req.method === 'POST' && path === '/api/commerce/inventory') { const x=await body(req);if(typeof x.shop_id!=='string'||!uuidPattern.test(x.shop_id)||typeof x.source_name!=='string'||!x.source_name.trim()||x.source_name.length>200||!Array.isArray(x.rows)||x.rows.length<1||x.rows.length>100)return fail('invalid_inventory');const rows=x.rows.map(commerceInventory);if(new Set(rows.map(row=>row.sku)).size!==rows.length)return fail('invalid_inventory');const {data,error}=await db.rpc('vco_upsert_inventory',{p_owner:user.id,p_shop:x.shop_id,p_source:x.source_name.trim(),p_rows:rows});if(error)return fail('inventory_failed',error.message.includes('shop_not_found')?404:400);return json({row_count:data},201) }
    if (req.method === 'GET' && path === '/api/commerce/inventory') { const shopId=url.searchParams.get('shop_id')||'',low=url.searchParams.get('low_stock')||'',format=url.searchParams.get('format')||'';if(shopId&&!uuidPattern.test(shopId))return fail('invalid_shop');if(low&&low!=='true')return fail('invalid_low_stock');if(format&&format!=='csv')return fail('invalid_format');let query=db.from('vco_inventory').select('id,shop_id,sku,available_quantity,reorder_point,is_low_stock,source_name,captured_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('updated_at',{ascending:false}).limit(200);if(shopId)query=query.eq('shop_id',shopId);if(low)query=query.eq('is_low_stock',true);const {data,error}=await query;if(error)throw error;if(format==='csv')return new Response(inventoryCsv(data||[]),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="vesper-commerce-low-stock.csv"','cache-control':'no-store'}});return json({inventory:data}) }
    if (req.method === 'GET' && path === '/api/commerce/inventory-summary') { const {data,error}=await db.rpc('vco_inventory_summary',{p_owner:user.id});if(error)throw error;return json({summary:data}) }
    if (req.method === 'GET' && path === '/api/commerce/inventory-exceptions') { const actionStatus=url.searchParams.get('action_status')||'',shopId=url.searchParams.get('shop_id')||'',format=url.searchParams.get('format')||'',states=new Set(['open','investigating','ignored']);if(actionStatus&&!states.has(actionStatus))return fail('invalid_exception_status');if(shopId&&!uuidPattern.test(shopId))return fail('invalid_shop');if(format&&format!=='csv')return fail('invalid_format');const {data,error}=await db.rpc('vco_inventory_exceptions',{p_owner:user.id,p_action_status:actionStatus||null,p_shop:shopId||null});if(error)return fail(error.message.includes('shop_not_found')?'shop_not_found':'inventory_exceptions_unavailable',error.message.includes('shop_not_found')?404:503);if(format==='csv')return new Response(inventoryExceptionCsv(data||[]),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="vesper-commerce-inventory-exceptions.csv"','cache-control':'no-store'}});return json({exceptions:data}) }
    if (req.method === 'POST' && path === '/api/commerce/inventory-exceptions') { const x=await body(req),states=new Set(['open','investigating','ignored']);if(typeof x.shop_id!=='string'||!uuidPattern.test(x.shop_id)||typeof x.sku!=='string'||!x.sku||x.sku.length>120||typeof x.status!=='string'||!states.has(x.status)||typeof x.note!=='string'||x.note.length>500)return fail('invalid_exception_action');const {data,error}=await db.rpc('vco_set_inventory_exception_action',{p_owner:user.id,p_shop:x.shop_id,p_sku:x.sku,p_status:x.status,p_note:x.note});if(error)return fail(error.message.includes('exception_not_found')?'exception_not_found':'exception_action_failed',error.message.includes('exception_not_found')?404:400);return json({action:data?.[0]}) }
    if (req.method === 'GET' && path === '/api/commerce/orders') { const shopId=url.searchParams.get('shop_id')||'',status=(url.searchParams.get('status')||'').trim(),canonicalStatus=url.searchParams.get('canonical_status')||'',from=url.searchParams.get('from')||'',to=url.searchParams.get('to')||'',format=url.searchParams.get('format')||'',canonicalStatuses=new Set(['pending_payment','processing','shipped','completed','cancelled','refunded','other']);if(shopId&&!uuidPattern.test(shopId))return fail('invalid_shop');if(status.length>80)return fail('invalid_status');if(canonicalStatus&&!canonicalStatuses.has(canonicalStatus))return fail('invalid_canonical_status');if((from&&!validDay(from))||(to&&!validDay(to))||(from&&to&&from>to))return fail('invalid_date_range');if(format&&format!=='csv')return fail('invalid_format');let query=db.from('vco_order_lines').select('id,shop_id,external_order_id,sku,quantity,amount_minor,currency,status,canonical_status,occurred_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('occurred_at',{ascending:false}).limit(200);if(shopId)query=query.eq('shop_id',shopId);if(status)query=query.eq('status',status);if(canonicalStatus)query=query.eq('canonical_status',canonicalStatus);if(from)query=query.gte('occurred_at',from+'T00:00:00.000Z');if(to)query=query.lt('occurred_at',new Date(Date.parse(to+'T00:00:00.000Z')+86400000).toISOString());const {data,error}=await query;if(error)throw error;if(format==='csv')return new Response(orderCsv(data||[]),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="vesper-commerce-orders.csv"','cache-control':'no-store'}});return json({orders:data}) }
