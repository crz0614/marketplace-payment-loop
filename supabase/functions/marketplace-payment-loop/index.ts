...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/inventory-exceptions') { const actionStatus=url.searchParams.get('action_status')||'',states=new Set(['open','investigating','ignored']);if(actionStatus&&!states.has(actionStatus))return fail('invalid_exception_status');const {data,error}=await db.rpc('vco_inventory_exceptions',{p_owner:user.id,p_action_status:actionStatus||null});if(error)throw error;return json({exceptions:data}) }
