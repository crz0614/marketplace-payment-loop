...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/inventory-exceptions') { const {data,error}=await db.rpc('vco_inventory_exceptions',{p_owner:user.id});if(error)throw error;return json({exceptions:data}) }
