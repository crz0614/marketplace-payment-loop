...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/imports') { const {data,error}=await db.from('vco_imports').select('id,shop_id,source_name,row_count,created_at').eq('owner_id',user.id).order('created_at',{ascending:false}).limit(100);if(error)throw error;return json({imports:data}) }
