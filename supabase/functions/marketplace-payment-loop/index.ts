...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/orders') { const {data,error}=await db.from('vco_order_lines').select('id,shop_id,external_order_id,sku,quantity,amount_minor,currency,status,occurred_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('occurred_at',{ascending:false}).limit(200);if(error)throw error;return json({orders:data}) }
