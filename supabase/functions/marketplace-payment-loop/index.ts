...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/inventory-exceptions') { const actionStatus=url.searchParams.get('action_status')||'',format=url.searchParams.get('format')||'',states=new Set(['open','investigating','ignored']);if(actionStatus&&!states.has(actionStatus))return fail('invalid_exception_status');if(format&&format!=='csv')return fail('invalid_format');const {data,error}=await db.rpc('vco_inventory_exceptions',{p_owner:user.id,p_action_status:actionStatus||null});if(error)throw error;if(format==='csv')return new Response(inventoryExceptionCsv(data||[]),{headers:{'content-type':'text/csv; charset=utf-8','content-disposition':'attachment; filename="vesper-commerce-inventory-exceptions.csv"','cache-control':'no-store'}});return json({exceptions:data}) }
