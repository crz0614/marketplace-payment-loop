...
const handleRequest = async (req: Request) => {
  try {
    ...
    if (req.method === 'GET' && path === '/api/commerce/orders') { const shopId=url.searchParams.get('shop_id')||'',status=(url.searchParams.get('status')||'').trim();if(shopId&&!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(shopId))return fail('invalid_shop');if(status.length>80)return fail('invalid_status');let query=db.from('vco_order_lines').select('id,shop_id,external_order_id,sku,quantity,amount_minor,currency,status,occurred_at,updated_at,shop:vco_shops(name,channel)').eq('owner_id',user.id).order('occurred_at',{ascending:false}).limit(200);if(shopId)query=query.eq('shop_id',shopId);if(status)query=query.eq('status',status);const {data,error}=await query;if(error)throw error;return json({orders:data}) }
