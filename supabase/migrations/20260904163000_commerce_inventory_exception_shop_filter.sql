drop function public.vco_inventory_exceptions(uuid,text);
...
begin
  ...
  if p_shop is not null and not exists (
  ...
  where lines.owner_id = p_owner
    and (p_shop is null or lines.shop_id = p_shop)
...
revoke all on function public.vco_inventory_exceptions(uuid,text,uuid)
  from public, anon, authenticated;
grant execute on function public.vco_inventory_exceptions(uuid,text,uuid)
  to service_role;

comment on function public.vco_inventory_exceptions(uuid,text,uuid) is
  'Owner-scoped order/inventory exceptions, optionally filtered by action status and an owned shop, capped at 200 groups.';

-- Rollback: drop the three-argument function and recreate the two-argument
-- vco_inventory_exceptions(uuid,text) implementation before redeploying v24.
